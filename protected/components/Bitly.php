 ...
 */
class BitlyError extends UserException
{}
...
/**
 * Visit => https://github.com/jsocol/bitly-api-php/blob/master/Bitly.php
 *
 * @param array $params
 *
 * @return string
...
class Bitly extends Component
{
     ...
     * @type string
     */
    public $loginUrl = 'https://bit.ly/oauth/authorize';
    ...
    /**
     * Prepare login URL for authenticate with Bitly
     *
     * @param string $redirectUrl Callback URL.
     * @param string $state State data to be retrun back to callback URL.
     * @return string $loginUrl Bitly authenticate URL.
     */
    public function getLoginUrl($redirectUrl, $state)
    {
        $loginUrl = $this->loginUrl . "?client_id={$this->clientId}&redirect_uri={$redirectUrl}&state={$state}";
        return $loginUrl;
    }
