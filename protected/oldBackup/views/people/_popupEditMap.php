...
<?php ActiveForm::end(); ?>
<?php
$updateKeyword_url = Yii::$app->urlManager->createUrl(['people/update-people-ajax-map', "id" => $model['id'], "check" => 2]);
$updateCategory_url = Yii::$app->urlManager->createUrl(['people/update-people-ajax-map', "id" => $model['id'], "check" => 4]);
$updateGrid_url = Yii::$app->urlManager->createUrl(['advanced-search/grid-update-map']);
$this->registerJs("
  $(document).ready(function () 
  {
        ...
        $('.updateCategory').on('click', function () 
        {
            ...
            $.post(urlCategory, {data: $('#form-edit').serialize()},
                function (data) 
                {
                    if (data) 
                    {
